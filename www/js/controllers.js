angular.module('corbynemoji.controllers', ['ionic.native'])
  .controller('Home', function($scope, $http, $ionicModal,
    ...
    function getLatest() {
      $http.get("http://jeremoji.codersforcorbyn.com/", {
          params: {
            json: 1,
            count: 10
          }
    ...
    function launchPage(post) {
      window.open(post.excerpt.replace(tagRegex, ""), '_system');
    }
    ...
    function donateClicked() {
      ...
      window.open('https://new-corbynstays.nationbuilder.com/donate',
        '_system');
    }
  ...
  .controller('EmojiCtrl', function($scope, $stateParams, Emoji,
    $cordovaSocialSharing, $cordovaFile, $ionicModal, $cordovaGoogleAnalytics,
    $ionicPlatform) {
    //Base image path
    ...
    $ionicPlatform.ready(function() {
      $cordovaGoogleAnalytics.trackView('Emoji Category: ' + $stateParams
        .category);
    });
    ...
    function share(shareObj) {
      var options = {
        ...
        files: [imgPath + shareObj.emoji.category + "/" + shareObj.emoji.name +
          ".png"
        ]
      };
      ...
      var shareSuccess = function(result) {
        ...
        if (result.app && result.app !== null && result.app !== "") {
          $cordovaGoogleAnalytics.trackEvent("Emoji", "Shared To", result.app,
            1);
        }

        $cordovaGoogleAnalytics.trackEvent("Emoji", "Shared Image",
          shareObj.emoji.category + "_" + shareObj.emoji.name, 1);
      };
      ...
      $cordovaSocialSharing.shareWithOptions(options, shareSuccess,
        shareError);
    }
